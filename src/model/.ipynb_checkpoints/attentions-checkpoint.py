...
class ST_Joint_Att1(nn.Module):
    def __init__(self, channel, reduct_ratio, bias, **kwargs):
        super(ST_Joint_Att1, self).__init__()
    ...
    def forward(self, x):
        N, V, T, C = x.size()
        #x = x.transpose(1, 3)#N,C,T,V
        x_t = x.mean(1, keepdims=True)
        x_v = x.mean(2, keepdims=True).transpose(1, 2)
        x_att = self.fcn(torch.cat([x_t, x_v], dim=2))
        x_t, x_v = torch.split(x_att, [T, V], dim=2)
        x_t_att = self.conv_t(x_t).sigmoid()
        x_v_att = self.conv_v(x_v.transpose(2, 3)).sigmoid()
        x_att = x_t_att * x_v_att
        x_att = x_att.transpose(1, 3)
        return x_att
